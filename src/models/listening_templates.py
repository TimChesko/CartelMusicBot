...
class ListeningTemplatesHandler:
        ...
        async with self.session_maker() as session:
            try:
                await session.execute(
                    delete(ListeningTemplates).where(ListeningTemplates.id == id)
                )
    ...
    async def get_all_all(self, type_name, id=None) -> str | bool:
        async with self.session_maker() as session:
            try:
    ...
    async def get_all_scalar(self, type_name, id=None) -> str | bool:
        async with self.session_maker() as session:
            try:
                if id is None:
                    query = select(ListeningTemplates).where(ListeningTemplates.type == type_name)
                else:
                    query = select(ListeningTemplates).where(ListeningTemplates.id == id)
                result = await session.execute(query)
    ...
    async def update_content(self, type: str, content: str, id=None):
        async with self.session_maker() as session:
            try:
                if id is None:
                    await session.execute(
                    ...
                    await session.execute(
                        update(ListeningTemplates).where(ListeningTemplates.id == id).values(content=content)
                    )
                await session.commit()
                return True
            except SQLAlchemyError as e:
