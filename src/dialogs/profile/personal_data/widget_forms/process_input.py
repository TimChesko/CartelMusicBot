import re
from typing import Any

from aiogram_dialog import DialogManager

from .window_input import start_dialog_filling_profile


async def start_input_form(
        btn_status: list[bool],
        task_list: dict,
        manager: DialogManager
) -> None:
    """
    Запускает форму ввода.

    :param btn_status: Статус кнопок.
                       id: 0 - остановка формы.
                       id: 1 - назад при вводе текста.
                       id: 2 - назад при выборе даты.
    :param header_data: Название типа данных, например, passport, bank, social.
    :param task_list: Список всех вопросов и ответов.
                      data_name: str - название данных.
                      text: str - вопрос в форме.
                      input: list - разрешенный тип данных.
    :param manager: Менеджер диалога.
    :return: None.
    """
    # Создаем список задач
    await dialog_save_task_list(btn_status, task_list, manager)
    # Запуск первой формы
    await start_dialog_filling_profile(*(await get_args_form(manager)))


async def get_args_form(manager: DialogManager) -> tuple[list[bool], bool, dict, DialogManager]:
    task_list = manager.dialog_data['task_list_all']
    key = manager.dialog_data["task_list_process"][0]
    input_date = True if "date" in task_list[key]['input_type'] else False
    btn_status = manager.dialog_data['btn_status']
    return btn_status, input_date, task_list[key], manager


async def process_input_result(_, result: Any, manager: DialogManager):
    save_input = manager.dialog_data['save_input']
    task_list = manager.dialog_data['task_list_all']
    task_list_done = manager.dialog_data["task_list_done"]
    task_list_process = manager.dialog_data["task_list_process"]

    input_type = task_list[task_list_process[0]]['input_type']

    if "back" == result[0]:
        if len(task_list_done) != 0:
            task_list_process.insert(0, task_list_done.pop())
            await start_dialog_filling_profile(*(await get_args_form(manager)))
        else:
            await manager.done()

    # elif "stop" == result[0]:
    #     # TODO проверить что тут будет
    #     await manager.done("cancel")

    elif "task_list_process" in manager.dialog_data:
        validate = await validate_input(input_type, result[0])
        if validate['check']:
            old_item = task_list_process.pop(0)
            if "date" in input_type:
                date = result[0]
                save_input[old_item] = date.strftime("%Y-%m-%d")
            else:
                save_input[old_item] = result[0]
            task_list_done.append(old_item)
            if len(manager.dialog_data["task_list_process"]) != 0:
                await start_dialog_filling_profile(*(await get_args_form(manager)))
            else:
                await manager.next()
        else:
            await start_dialog_filling_profile(*(await get_args_form(manager)), error=str(validate['value']))


async def dialog_save_task_list(
        btn_status: list[bool],
        task_list: dict,
        manager: DialogManager
) -> None:
    manager.dialog_data['btn_status'] = btn_status
    manager.dialog_data['save_input'] = {}
    manager.dialog_data['task_list_all'] = task_list
    manager.dialog_data['task_list_done'] = []
    manager.dialog_data["task_list_process"] = list(task_list.keys())


async def validate_input(input_type: list[str], input_result: str) -> dict[Any, bool]:
    template_input = {
        "int": ["\d{1,9}", "число до 9 цифр"],
        "big_int": ["\d{1,20}", "число до 20 цифр"],
        "text": ["a-zA-Zа-яА-Я", "буквы"],
        "punctuation": [",.", "точки, запятые"],
        "minus": ["-", "тире"],
        "space": ["\s", "пробелы"],
        "any": [".*", "любые символы"],
        "date": [".*", "любые символы"],
    }

    input_pattern = ''.join(template_input.get(item, [""])[0] for item in input_type)
    allowed_characters = ", ".join(template_input.get(item, ["", ""])[1] for item in input_type)

    if "any" in input_type:
        regex_pattern = rf'{input_pattern}$'
    elif ("big_int" or "int") in input_type:
        regex_pattern = rf'^{input_pattern}$'
    else:
        regex_pattern = rf'^[{input_pattern}]+$'

    if "date" in input_type or re.match(regex_pattern, input_result):
        return {"value": input_result, "check": True}
    else:
        return {"value": f"Ответ может содержать {allowed_characters}\nПовторите попытку снова", "check": False}
