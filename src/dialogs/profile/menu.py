...
from src.utils.fsm import Profile, Passport, Bank


async def get_data(dialog_manager: DialogManager, **kwargs):
    data = dialog_manager.middleware_data
    user_id = data['event_from_user'].id
    passport_id, bank_id = await PersonalDataHandler(data['engine'], data['database_logger']).get_all_data_status(user_id)
...
menu = Dialog(
    Window(
        ...
        Start(Const("Изменить паспортные данные"),
              id="profile_edit_passport",
              when="edit_passport",
              state=Passport.edit_data),
