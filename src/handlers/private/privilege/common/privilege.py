    ...
    match command:
            ...
            try:
                ...
                if tg_id != msg.from_user.id:
                    if await CheckPrivilege(privilege_str).simple(engine, database_logger, msg, config):
                        ...
                        await msg.answer(f"Привелегия {privilege_str} присвоена пользователю {tg_id}")
                    else:
                        await msg.answer("Данной привилегии не существует или "
                                         "ваша привилегии ниже привилегии которую вы хотите присвоить")
                else:
                    await msg.answer(f"В целях безопастности, вы не можете изменить свою роль")
            except ValueError:
        ...
        case _:
            await msg.answer("Доступные команды:\n\n"
                             ...
                             "/privilege set {id} {privilege} - присвоить привелегию пользователю\n"
                             "/privilege del {id} - отобрать привелегию у пользователя\n")
